...
| `argus` | disabled | Content security / compliance scanning. Before enabling: add the 5 `ARGUS_*` keys to `secrets-vars.yaml` and 2 to `vars.yaml` — they are replacement sources, so a missing key fails the whole render. As wired here `ARGUS_DATABASE_URL` is a static credential that RDS rotation will not update; argus does support Secrets Manager (empty DSN + `RDS_SECRET_ARN` + IRSA/Pod Identity), but the replacement targets for it ship in the phoenix overlay. List it **before** `nonroot-security-context`. |
