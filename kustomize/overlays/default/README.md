...
| `argus` | disabled | Content security / compliance scanning. Before enabling: add the 5 `ARGUS_*` keys to `secrets-vars.yaml` and 2 to `vars.yaml` — they are replacement sources, so a missing key fails the whole render. List it **before** `nonroot-security-context`. |
